...
class ImgProjector(nn.Module):
    ...
    def __init__(self, config, type="full"):
        super().__init__()
        inp_dim = 2048 if type == "full" else 64
...
class MultimodalTransformerEncoder(MBartEncoder):
    def __init__(self, config: MBartConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        # encoder_layer = nn.TransformerEncoderLayer(d_model=1024, nhead=16)
        # self.transformer_encoder = nn.TransformerEncoder(encoder_layer, num_layers=12)

        self.copied_layer = copy.deepcopy(self.layers)
        self.common_layer = self.copied_layer

        self.diffloss = DiffLoss()
        self.gating = GatingMechanism()

        self.gatedFusion = nn.Sequential(
            nn.Linear(2048, 1024),
            nn.Sigmoid()
        )
    ...
    def forward(
        ...
        textSpecific = self.layernorm_embedding(hidden_states)
        ...
        if img_features["global_features"] is not None:
            ...
            img_feats = img_feats + (full_img_embeds,)

        if len(img_feats):
            all_feats = img_feats + (hidden_states,)
            hidden_states = torch.cat(all_feats, dim=1)

        imageFeature_padding_mask = torch.sum(img_feats[0], dim=-1).eq(0).int()
        textImageMask = torch.cat((attention_mask[:, :-1], imageFeature_padding_mask), dim=1)

        textSpecific = nn.functional.dropout(textSpecific, p=self.dropout, training=self.training)
        hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
        commontextMask = attention_mask
        # expand attention_mask
        if attention_mask is not None:
            if len(attention_mask.size()) == 2:
                # [bsz, seq_len] -> [bsz, 1, tgt_seq_len, src_seq_len]
                attention_mask = _expand_mask(attention_mask[:, :-1], inputs_embeds.dtype)
                textImageMask = _expand_mask(textImageMask, img_feats[0].dtype)
            else:
        ...
        #common
        # textImageMask = torch.cat((commontextMask, imageFeature_padding_mask), dim=1)
        # commonFeatures = self.transformer_encoder(src=hidden_states,mask=textImageMask)
        # CommonText = commonFeatures[:, :textSpecific.shape[0], :]
        # CommonImage = commonFeatures[:, -49:, :]
        for idx, encoder_layer in enumerate(self.common_layer):
            ...
            if self.training and (dropout_probability < self.layerdrop):
                ...
                commonFeatures = layer_outputs[0]
        common_hidden_state = self.layer_norm(commonFeatures)
        CommonText = common_hidden_state[:, :textSpecific.shape[1], :]
        CommonImage = common_hidden_state[:, -img_feats[0].shape[0]:, :]
        #specific
        for idx, encoder_layer in enumerate(self.layers):
            ...
            if self.training and (dropout_probability < self.layerdrop):  # skip the layer
                ...
                textSpecific = layer_outputs[0]

            if output_attentions:
                all_attentions = all_attentions + (layer_outputs[1],)

        text_hidden_states = self.layer_norm(textSpecific)

        if output_hidden_states:
            encoder_states = encoder_states + (text_hidden_states,)

        specialT = torch.mean(text_hidden_states, dim=1)
        commonT = torch.mean(CommonText, dim=1)
        commonImage = torch.mean(CommonImage, dim=1)
        misaloss = self.diffloss(commonT, specialT)

        NCEC = torch.tensor(0)
        logitsC = commonT @ commonImage.transpose(0, 1)
        labelsC = torch.arange(len(commonT)).cuda()
        for i in range(len(logitsC)):
            NCEC = F.cross_entropy(logitsC[i] / 0.1, labelsC[i]) + NCEC
        NCEC = NCEC / len(labelsC)

        loss = NCEC + misaloss
        src_img_features, gate = self.gating(CommonText.transpose(0, 1), CommonImage.transpose(0, 1))
        xCommon = CommonText + src_img_features.transpose(0, 1)

        textCP = []
        for i in range(len(text_hidden_states)):
            gateE = self.gatedFusion(torch.cat((xCommon[i], text_hidden_states[i]), dim=1))
            textCP.append((1 - gateE) * xCommon[i] + gateE * text_hidden_states[i])
        textCP = torch.stack(textCP)

        if not return_dict:
            return tuple(v for v in [textCP, 5*loss, encoder_states, all_attentions] if v is not None)

        return BaseModelOutput(
            last_hidden_state=textCP, hidden_states=encoder_states, attentions=all_attentions
        )
    ...
    def forward(
        ...
        # If the user passed a tuple for encoder_outputs, we wrap it in a BaseModelOutput when return_dict=True
            ...
            encoder_outputs = BaseModelOutput(
                ...
                loss=encoder_outputs[1] if len(encoder_outputs) > 1 else None,
...
class VGAMT(MBartForConditionalGeneration):
    ...
    def forward(
        ...
        if not return_dict:
            ...
            masked_lm_loss = masked_lm_loss + outputs[2]
...
class GatingMechanism(nn.Module):
    def __init__(self):
        super().__init__()
        self.fc_img = nn.Linear(1024 * 2, 1)
